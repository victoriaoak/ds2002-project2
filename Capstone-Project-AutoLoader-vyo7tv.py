...
# MAGIC Notebook for the DS-2002 capstone project. This data project builds upon the first project, implementing data science systems such as relational/NoSQL databases and ETL process pipelines, while also utilizing cloud services that can support data lakehouse architecture.
# MAGIC 
# MAGIC This project uses the Sakila database, which represents a business process of the interaction between customers and retailers that is reflected in movie rental transactions.
# MAGIC 
# MAGIC **These include:**
...
jdbc_hostname = "ds2002-sql.database.windows.net"
jdbc_port = 1433
src_database = "SakilaLT"
...
# MAGIC %sql
# MAGIC DROP DATABASE IF EXISTS sakila CASCADE;

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE DATABASE IF NOT EXISTS sakila
# MAGIC COMMENT "Capstone Project Database"
# MAGIC LOCATION "dbfs:/FileStore/ds2002-capstone/sakila"
...
# MAGIC OPTIONS (
# MAGIC   url "jdbc:sqlserver://ds2002-sql.database.windows.net:1433;database=SakilaLT",
# MAGIC   dbtable "RentalsLT.vDimCustomers",
# MAGIC   user "vok",
...
# MAGIC %sql
# MAGIC USE DATABASE sakila;
# MAGIC 
# MAGIC CREATE TABLE IF NOT EXISTS sakila.dim_customers
# MAGIC COMMENT "Customer Dimension Table"
# MAGIC LOCATION "dbfs:/FileStore/ds2002-capstone/sakila/dim_customers"
# MAGIC AS SELECT * FROM view_customers

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM sakila.dim_customers LIMIT 5

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED sakila.dim_customers;
...
# MAGIC USE DATABASE sakila;
# MAGIC 
# MAGIC CREATE TABLE IF NOT EXISTS sakila.dim_date
# MAGIC COMMENT "Date Dimension Table"
# MAGIC LOCATION "dbfs:/FileStore/ds2002-capstone/sakila/dim_date"
# MAGIC AS SELECT * FROM view_date

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM sakila.dim_date LIMIT 5

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED sakila.dim_date;
...
json_files = {"inventory" : 'SakilaLT_DimInventory.json'}
...
# MAGIC %scala
# MAGIC df_films.write.format("delta").mode("overwrite").saveAsTable("sakila.dim_inventory")

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED sakila.dim_inventory
...
# MAGIC %sql
# MAGIC SELECT * FROM sakila.dim_inventory LIMIT 5
...
staff_csv = f"{batch_dir}/SakilaLT_DimStaff.csv"
...
df_staff.write.format("delta").mode("overwrite").saveAsTable("sakila.dim_staff")

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED sakila.dim_staff;

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM sakila.dim_staff LIMIT 5;
...
# MAGIC   INNER JOIN sakila.dim_address sa
# MAGIC   ON t.ShipToAddressID = CAST(sa.AddressID AS BIGINT)
# MAGIC   INNER JOIN sakila.dim_address ba
# MAGIC   ON t.BillToAddressID = CAST(ba.AddressID AS BIGINT)
# MAGIC   INNER JOIN sakila.dim_product p
...
# MAGIC   INNER JOIN sakila.dim_date sd
...
# MAGIC FROM sakila.fact_orders_silver
...
# MAGIC FROM sakila.fact_orders_silver AS os
# MAGIC INNER JOIN (
# MAGIC   SELECT CustomerID
# MAGIC   , COUNT(ProductID) AS ProductCount
# MAGIC   FROM sakila.fact_orders_silver
